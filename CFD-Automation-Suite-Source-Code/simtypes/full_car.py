...
Geometry: the whole car, no symmetry plane. All four wheels are modelled
individually (flw/frw/rlw/rrw), so forces are read directly and are never
doubled the way a half-car model's are.

Coordinate convention
    +X  toward the rear of the car = flow direction   -> drag      [ 1, 0, 0]
    +Y  up                                            -> downforce [0, -1, 0]
    +Z  driver's left                                 -> right side at z < 0
...
NAMED_SELECTIONS = {
    ...
    # No symmetry plane -- the whole car is modelled.
    ...
    "flw":             ("wall, rotating",  True,  "Front left wheel"),
    "flwb":            ("wall, rotating",  True,  "Front left wheel block"),
    "frw":             ("wall, rotating",  True,  "Front right wheel"),
    "frwb":            ("wall, rotating",  True,  "Front right wheel block"),
    "rlw":             ("wall, rotating",  True,  "Rear left wheel"),
    "rlwb":            ("wall, rotating",  True,  "Rear left wheel block"),
...
REAR_LEFT_WHEEL   = ["rlw", "rlwb"]
REAR_RIGHT_WHEEL  = ["rrw", "rrwb"]
...
def _boundary_conditions(session, s: Settings, log) -> None:
    ...
    # front_wheel_origin / rear_wheel_origin hold the RIGHT side (+Z) point;
    # the left side is the same point mirrored to -Z.
    omega = s.wheel_omega
    ...
    for labels, origin, corner in (
        (FRONT_RIGHT_WHEEL, s.front_wheel_origin,      "front-right"),
        (FRONT_LEFT_WHEEL,  _mirror(s.front_wheel_origin), "front-left"),
        (REAR_RIGHT_WHEEL,  s.rear_wheel_origin,       "rear-right"),
        (REAR_LEFT_WHEEL,   _mirror(s.rear_wheel_origin),  "rear-left"),
    ):
...
def _report_definitions(session, s: Settings, area: float, log) -> None:
    ...
    # Full car: fz and fx already cover both sides, so no doubling here.
    q = s.dynamic_pressure
    expression("SCz", f"abs(fz) / {q}")
    expression("SCx", f"abs(fx) / {q}")
...
def _read_reports(session, s: Settings, area: float, log) -> dict:
    ...
    # Full car: both sides are already in the mesh, so no doubling.
    MULT = 1.0
